...
def XG_home(match_id,HomeTeam_name):
    ...
    Shots_home=events[(events['type_name']=='Shot') & (events['team_name']==f'{HomeTeam_name}') ]
    ...
    return xG_home_sum
    #return {'0': xG_home_sum, '1': xG_home_sum}
 

def XG_away(match_id,AwayTeam_name):
    
    #match_id=3795506
    #AwayTeam_name='England'
    with open(rf'C:\Users\hp\Desktop\Script python\Calcio\Da modificare\Con Messi e euro2020\open-data-master\data\events\{match_id}.json') as data_file:
        #print (mypath+'events/'+file)
        data = json.load(data_file)
    ...
    Shots_away=events[(events['type_name']=='Shot') & (events['team_name']==f'{AwayTeam_name}') ]
...
def Most_involved(match_id,HomeTeam_name,AwayTeam_name):
    ...
    Filt=Filt[(Filt['possession_team_name']==AwayTeam_name) & (Filt['period']!=5) ]
    ...
    Filt_home=Filt_home[(Filt_home['possession_team_name']==HomeTeam_name) & (Filt_home['period']!=5)]
    ...
    if Max>Max_home:
       #print(f'Most involved player:{player} with {Max} xG')
       return f'Most involved player:{player} with {Max} xG'
    else:
       # print(f'Most involved player:{player_home} with {Max_home} xG')
